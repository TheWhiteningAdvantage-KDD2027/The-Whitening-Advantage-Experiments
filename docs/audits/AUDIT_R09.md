...
8. SHARED ORCHESTRATOR TEST REINFORCEMENT: `test_R09_the_shared_orchestrators_are_untouched` was amended in response to a repository-level change. Because `run_all.sh` abandoned sorted glob enumeration for an explicit topological order derived from the DAG, the proxy test asserting the strict absence of `"R09"` failed. It was replaced by a strictly stronger structural test verifying that R09 appears exactly once in the order list, without hard-coded execution slugs or specific arguments. No experimental parameter, seed, statistical tolerance, or bound was altered.
