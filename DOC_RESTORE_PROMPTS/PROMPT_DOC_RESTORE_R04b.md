# DOCUMENTATION RESTORATION — STREAM R04b

Repository root: `/home/m53/The-Whitening-Advantage-Experiments`. All paths below are relative to it.

You restore two documents for stream **R04b** (slug `R04b_nu_refinement`). The experimental artefacts,
...
1. `logs/R04b_nu_refinement/*.log` — the execution log. **Source of truth for every number.** Read it
   in full before writing anything.
2. `results/R04b_nu_refinement/data/*.csv` and `results/R04b_nu_refinement/tables/R04b_claims.tex` — for
   locating the source cell of a value. Read CSVs with `float_precision='round_trip'` if you read
   them programmatically.
3. `docs/audits/AUDIT_R04b.md` and `docs/sections/R04b.md` — current drafts. Use them for
   structure and for prose you may keep. **Never use them as a source for a number**: they are
   what you are repairing.
4. `docs/DEVIATIONS.md`, section R04b — cross-check only.
...
## §2. DELIVERABLE 1 — `docs/audits/AUDIT_R04b.md`
...
The `source CSV cell` names a file and a column, e.g. `R04b_foo.csv :: bar_rate`. The `log line`
...
### `## 3. Test suite`
Run `pytest tests/test_R04b_claims.py -v` and paste the output **verbatim** inside a fenced block.
...
`sha256sum results/R04b_nu_refinement/data/*.csv results/R04b_nu_refinement/tables/*.tex` and paste the current
...
3. The reproduction command: `bash run_experiment_R04b.sh`.
...
grep -Ein 'proves|proven|perfectly valid|validates the (theorem|thesis|claim)|confirms the|as expected|triumph|victory|irrefutable|brilliant' \
  docs/audits/AUDIT_R04b.md docs/sections/R04b.md
grep -in 'wilson' docs/audits/AUDIT_R04b.md      # every hit must be on a proportion
grep -in 'deff *= *1\|simple random sampling'      docs/audits/AUDIT_R04b.md
grep -in 'BLAS\|associativity\|summation order'    docs/audits/AUDIT_R04b.md docs/sections/R04b.md
grep -c 'D3' docs/audits/AUDIT_R04b.md            # compare against the log
tail -c 1 docs/audits/AUDIT_R04b.md | od -c | head -1   # must end with \n
```
...
or any `.tex` or `.bib`. Two files only: `docs/audits/AUDIT_R04b.md` and `docs/sections/R04b.md`.
