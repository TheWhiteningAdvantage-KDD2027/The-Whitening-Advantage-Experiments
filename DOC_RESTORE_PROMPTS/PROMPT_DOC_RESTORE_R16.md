# DOCUMENTATION RESTORATION — STREAM R16

Repository root: `/home/m53/The-Whitening-Advantage-Experiments`. All paths below are relative to it.

You restore two documents for stream **R16** (slug `R16_regime_census`). The experimental artefacts,
...
1. `logs/R16_regime_census/*.log` — the execution log. **Source of truth for every number.** Read it
   in full before writing anything.
2. `results/R16_regime_census/data/*.csv` and `results/R16_regime_census/tables/R16_claims.tex` — for
   locating the source cell of a value. Read CSVs with `float_precision='round_trip'` if you read
   them programmatically.
3. `docs/audits/AUDIT_R16.md` and `docs/sections/R16.md` — current drafts. Use them for
   structure and for prose you may keep. **Never use them as a source for a number**: they are
   what you are repairing.
4. `docs/DEVIATIONS.md`, section R16 — cross-check only.
...
## §2. DELIVERABLE 1 — `docs/audits/AUDIT_R16.md`
...
The `source CSV cell` names a file and a column, e.g. `R16_foo.csv :: bar_rate`. The `log line`
...
### `## 3. Test suite`
Run `pytest tests/test_R16_claims.py -v` and paste the output **verbatim** inside a fenced block.
...
`sha256sum results/R16_regime_census/data/*.csv results/R16_regime_census/tables/*.tex` and paste the current
...
3. The reproduction command: `bash run_experiment_R16.sh`.
...
grep -Ein 'proves|proven|perfectly valid|validates the (theorem|thesis|claim)|confirms the|as expected|triumph|victory|irrefutable|brilliant' \
  docs/audits/AUDIT_R16.md docs/sections/R16.md
grep -in 'wilson' docs/audits/AUDIT_R16.md      # every hit must be on a proportion
grep -in 'deff *= *1\|simple random sampling'      docs/audits/AUDIT_R16.md
grep -in 'BLAS\|associativity\|summation order'    docs/audits/AUDIT_R16.md docs/sections/R16.md
grep -c 'D3' docs/audits/AUDIT_R16.md            # compare against the log
tail -c 1 docs/audits/AUDIT_R16.md | od -c | head -1   # must end with \n
```
...
or any `.tex` or `.bib`. Two files only: `docs/audits/AUDIT_R16.md` and `docs/sections/R16.md`.
