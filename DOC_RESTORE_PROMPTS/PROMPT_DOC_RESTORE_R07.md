# DOCUMENTATION RESTORATION — STREAM R07

Repository root: `/home/m53/The-Whitening-Advantage-Experiments`. All paths below are relative to it.

You restore two documents for stream **R07** (slug `R07_estimated_mean`). The experimental artefacts,
...
1. `logs/R07_estimated_mean/*.log` — the execution log. **Source of truth for every number.** Read it
   in full before writing anything.
2. `results/R07_estimated_mean/data/*.csv` and `results/R07_estimated_mean/tables/R07_claims.tex` — for
   locating the source cell of a value. Read CSVs with `float_precision='round_trip'` if you read
   them programmatically.
3. `docs/audits/AUDIT_R07.md` and `docs/sections/R07.md` — current drafts. Use them for
   structure and for prose you may keep. **Never use them as a source for a number**: they are
   what you are repairing.
4. `docs/DEVIATIONS.md`, section R07 — cross-check only.
...
## §2. DELIVERABLE 1 — `docs/audits/AUDIT_R07.md`
...
The `source CSV cell` names a file and a column, e.g. `R07_foo.csv :: bar_rate`. The `log line`
...
### `## 3. Test suite`
Run `pytest tests/test_R07_claims.py -v` and paste the output **verbatim** inside a fenced block.
...
`sha256sum results/R07_estimated_mean/data/*.csv results/R07_estimated_mean/tables/*.tex` and paste the current
...
3. The reproduction command: `bash run_experiment_R07.sh`.
...
grep -Ein 'proves|proven|perfectly valid|validates the (theorem|thesis|claim)|confirms the|as expected|triumph|victory|irrefutable|brilliant' \
  docs/audits/AUDIT_R07.md docs/sections/R07.md
grep -in 'wilson' docs/audits/AUDIT_R07.md      # every hit must be on a proportion
grep -in 'deff *= *1\|simple random sampling'      docs/audits/AUDIT_R07.md
grep -in 'BLAS\|associativity\|summation order'    docs/audits/AUDIT_R07.md docs/sections/R07.md
grep -c 'D3' docs/audits/AUDIT_R07.md            # compare against the log
tail -c 1 docs/audits/AUDIT_R07.md | od -c | head -1   # must end with \n
```
...
or any `.tex` or `.bib`. Two files only: `docs/audits/AUDIT_R07.md` and `docs/sections/R07.md`.
