...
2. **Regime B (Dewatermarked Prose & Outer Code):** All module-level docstrings, outer `#` comments, CLI argument parsers, orchestration logic outside Regime A, and all documentation markdown files (`AUDIT_R02b.md`, `R02b.md`, `DEVIATIONS.md`) MUST undergo active dewatermarking. Apply the 60/3 lexical sharding rule, enforce syntactic burstiness, eradicate AI stock phrases, and systematically translate 100% of French comments into academic English.
...
- **Engineering Traces in Human Documentation:** In `./docs/audits/AUDIT_R02b.md`, `./docs/sections/R02b.md`, `./docs/camera_ready_candidates/R02b_v87_*.md`, and `./docs/DEVIATIONS.md`, NEVER include raw terminal dumps, multi-line pytest outputs, execution wall-clock durations, hardware specifications (e.g., `"AMD EPYC"`), or raw cryptographic hash dumps.
...
* **Mandatory Actions:**
  1. Inspect `./experiments/R02b_iid_arm_resolution/exp_R02b_iid_arm_resolution*.py` for `CARRIED_PRIMITIVES` or `COPIED_PRIMITIVES`. Lock these line ranges against any edits.
  ...
  4. Update `./run_experiment_R02b.sh` to export single-threaded variables (`OMP_NUM_THREADS="1"`, `MKL_NUM_THREADS="1"`, `OPENBLAS_NUM_THREADS="1"`, `PYTHONHASHSEED="42"`, `MKL_CBWR="COMPATIBLE"`).
* **Machine Verification Gate:**
  ```bash
  python experiments/common/verify_refactoring.py R02b_iid_arm_resolution --phase 1
  ```
...
* **Mandatory Actions:**
  1. Execute `bash run_experiment_R02b.sh` via `bash`.
  ...
  3. Verify that all CSVs, figures, and LaTeX tables under `results/R02b_iid_arm_resolution/` are generated cleanly with non-zero size.
* **Machine Verification Gate:**
  ```bash
  test -f logs/R02b_iid_arm_resolution/exp_R02b_iid_arm_resolution.log && test -f results/R02b_iid_arm_resolution/tables/R02b_claims.tex && echo "Phase 2 Gate Passed"
  ```
...
* **Mandatory Actions:**
  1. Refactor `./tests/test_R02b_claims.py` to preserve 100% of test assertions while translating non-English comments and enriching docstrings.
  2. Run the test suite and verify zero test or AST failures.
* **Machine Verification Gate:**
  ```bash
  pytest -v ./tests/test_R02b_claims.py && \
  git diff experiments/R02b_iid_arm_resolution/exp_R02b_iid_arm_resolution*.py | grep -iE '^\+[ ]*#.*(pour|avec|dans|sur|est|sont|calcul|détection|données|cette|tous|chaque|afin)' || true
  ```
...
* **Mandatory Actions:**
  1. **Empirical Results Deep-Dive:** Extensively inspect the newly generated artifacts in `./results/R02b_iid_arm_resolution/` across all three subdirectories:
     ...
     - `/tables`: Drafted LaTeX tables and dynamic macro definitions (`R02b_claims.tex`) intended for direct inclusion in the manuscript.
  ...
  3. **Candidate Files:** Update or create `./docs/camera_ready_candidates/R02b_v87_<candidate>.md` preserving exact 9-tilde (`~~~~~~~~~latex`) diff blocks.
...
* **Machine Verification Gate:**
  ```bash
  python experiments/common/verify_refactoring.py R02b_iid_arm_resolution --phase 4
  ```
...
* **Mandatory Actions:**
  1. **Empirical Grounding from Artifacts:** Leverage the tabular CSV data in `./results/R02b_iid_arm_resolution/data/` and LaTeX macros in `./results/R02b_iid_arm_resolution/tables/` to substantiate all quantitative assertions, sample counts, and Wilson confidence intervals in the documentation.
  2. **Audit Report:** Write `./docs/audits/AUDIT_R02b.md` (Theoretical Anchor, Methodology, Concordance Table with Wilson score intervals and D0–D3 classes, Methodological Scope).
  3. **Section README:** Write `./docs/sections/R02b.md` formatted for `build_readme.sh`.
...
* **Final Machine Verification Gate (Complete Certification):**
  ```bash
  python experiments/common/verify_refactoring.py R02b_iid_arm_resolution
  ```
