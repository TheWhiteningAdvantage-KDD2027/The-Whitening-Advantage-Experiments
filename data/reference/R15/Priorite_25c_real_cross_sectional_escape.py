...
def run_real_experiment(args, logger):
    ...
    df_a.to_csv(FIGURES_DIR / "protocol_25c_real_panel_diagnostics.csv", index=False)
    ...
    df_b.to_csv(FIGURES_DIR / "protocol_25d_real_cross_sectional_race.csv", index=False)
    ...
    if idx_onset - H_ref >= 0:
        ...
        for K in K_GRID:
            ...
            if delay_boot != -1:
                det_date = dates[idx_onset + delay_boot].date()
    ...
    ax.plot(df_a['K'], np.sqrt(df_a['K_eff_meas']), marker='', linestyle=':', color='black', label='Limit: sqrt(K_eff)')
    ax.plot(df_a['K'], df_a['K_eff_meas'], marker='', linestyle='-.', color='gray', alpha=0.5, label='False Claim: Linear K_eff')
    ax.set_xscale('log')
    ax.set_xlabel('Panel Size K (Real Data)')
    ...
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "Fig30_RealCrossSectional_Escape.png")
    plt.close()
...
def setup_logger():
    ...
    fh = logging.FileHandler(BASE_DIR / "Priorite_25c_real_cross_sectional_escape.log", mode='w')
...
def run_experiment():
        ...
        for K in K_GRID:
            for rho in RHO_GRID:
                ...
                if not (0.80 <= keff_ratio <= 1.20):
                    ...
                    pd.DataFrame(res_a).to_csv(FIGURES_DIR / "protocol_25a_panel_diagnostics.csv", index=False)
                    sys.exit(1)
    ...
    df_a = pd.DataFrame(res_a)
    df_a.to_csv(FIGURES_DIR / "protocol_25a_panel_diagnostics.csv", index=False)
    ...
    df_b = pd.DataFrame(res_b)
    df_b.to_csv(FIGURES_DIR / "protocol_25b_cross_sectional_race.csv", index=False)
    ...
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "Fig29_CrossSectional_Escape.png")
    plt.close()
